...
def get_critique_neg_loss_gateaware(user_embed, neg_embed, gate_pen, mask=None, gate=None):
    """Gate-aware neg BPR: subtract the LLM gate penalty gate_pen from neg_score so the residual gradient is suppressed on FP items the inference gate already handles."""
    neg_score = torch.sum(user_embed.unsqueeze(1) * neg_embed, dim=2) - gate_pen
    per_elt = -torch.log(1 - torch.sigmoid(neg_score) - 1e-8)
    if gate is not None:
        denom = gate.sum() + 1e-8
        return (per_elt * gate).sum() / denom
...
def get_semantic_offset_bpr_loss(user_embed, pos_embed, neg_embed, q_neg, tau=1.0, neg_mask=None):
    """Semantic-Offset L_cri: pairwise joint-margin BPR (CL anchor vs critique dislike) on joint score s'=s_delta+q."""
    s_pos = torch.sum(user_embed * pos_embed, dim=1, keepdim=True)
    s_neg = torch.sum(user_embed.unsqueeze(1) * neg_embed, dim=2)
    delta = (s_pos - s_neg) - q_neg
    per_pair = -torch.log(torch.sigmoid(delta / tau) + 1e-8)
    if neg_mask is not None:
        m = neg_mask.float().unsqueeze(1)
...
def get_cer_loss(user_embed, pos_embed, crit_neg_embed, rand_neg_embed, tau=1.0):
    ...
    pos_score = torch.sum(user_embed * pos_embed, dim=1)
    crit_score = torch.sum(user_embed.unsqueeze(1) * crit_neg_embed, dim=2)
    rand_score = torch.sum(user_embed.unsqueeze(1) * rand_neg_embed, dim=2)
    num = torch.exp(pos_score / tau)
...
def get_cer_margin_loss(user_embed, pos_embed, rand_neg_embed, cri_neg_embed, cri_neg_g,
                        margin=0.5, tau=0.1):
    """s_L-margin CER: add an additive margin m*g_n to the dislike bucket B- logits in the InfoNCE denominator."""
    s_pos = torch.sum(user_embed * pos_embed, dim=1)
    s_rand = torch.sum(user_embed.unsqueeze(1) * rand_neg_embed, dim=2)
    s_cri = torch.sum(user_embed.unsqueeze(1) * cri_neg_embed, dim=2)
    valid_cri = (cri_neg_g > 0).float()
    num = torch.exp(s_pos / tau)
...
def get_ccr_loss(user_embed, pos_embed, rand_neg_embed, cri_neg_embed, cri_neg_q, tau=1.0):
    """Unified CCR: set-wise Z+/Z- contrast on the joint score z=s_delta+q (dislike bucket decreased by q<0)."""
    s_pos = torch.sum(user_embed * pos_embed, dim=1)
    s_rand = torch.sum(user_embed.unsqueeze(1) * rand_neg_embed, dim=2)
    s_cri = torch.sum(user_embed.unsqueeze(1) * cri_neg_embed, dim=2)
    z_cri = s_cri + cri_neg_q
    valid_cri = (cri_neg_q < 0).float()
    num = torch.exp(s_pos / tau)
    den = num + torch.exp(s_rand / tau).sum(dim=1) \
...
def get_residual_llm_loss_reg(res_embed, pool_embed, pool_scores):
    ...
    score = torch.sum(res_embed.unsqueeze(1) * pool_embed, dim=2)
    real = (pool_scores != 0).float()
    cnt = real.sum(dim=1, keepdim=True)
    ...
    target = (pool_scores - mu) / sigma
    se = (score - target) ** 2 * real
    denom = real.sum() + 1e-8
    return se.sum() / denom
...
def get_residual_llm_loss_pair(res_embed, pool_embed, pool_scores, tau=0.05):
    ...
    score = torch.sum(res_embed.unsqueeze(1) * pool_embed, dim=2)
    real = (pool_scores != 0).float()
    sdiff = pool_scores.unsqueeze(2) - pool_scores.unsqueeze(1)
    score_diff = score.unsqueeze(2) - score.unsqueeze(1)
    valid = (sdiff > tau).float() * real.unsqueeze(2) * real.unsqueeze(1)
    loss_elt = -torch.log(torch.sigmoid(score_diff) + 1e-8)
...
def get_residual_llm_loss_pair_v2_polarity(score_prop, score_neg, pool_scores,
    ...
    """Form B-v2 polarity-aware: pos members use propagated score_prop, neg members use non-propagated score_neg."""
    real = (pool_scores != 0).float()
    pos_m = (pool_scores > 0).float()
    neg_m = (pool_scores < 0).float()
    score = pos_m * score_prop + neg_m * score_neg
    sdiff = pool_scores.unsqueeze(2) - pool_scores.unsqueeze(1)
    score_diff = score.unsqueeze(2) - score.unsqueeze(1)
    p_ij = torch.sigmoid(beta * sdiff) if beta > 0 else (sdiff > 0).float()
    if within_w < 1.0:
...
def get_residual_orth_loss(res_embed, anchor_embed):
    """(a) Orthogonal protection: L_prot=mean(<delta_u, phi_pos_bar>^2) drives residual projection on the train-positive-mean direction to zero."""
    proj = torch.sum(res_embed * anchor_embed, dim=1)
