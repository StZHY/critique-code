...
def testing_100(model, args, dataset, device):
    ...
    with torch.no_grad():
        test_users = list(dataset.test_dict.keys())[:100]
        user_list, true_list, rating_list = [], [], []
        num_batch = len(test_users) // int(args.test_batch_size) + 1
...
def test_single_batch(single_list, topK):
    ...
    for k_size in topK:
        ...
        hitrate.append(hitrate_k(pred_item_label, k_size))
    return {'recall': np.array(recall), 'ndcg': np.array(ndcg), 'hitrate': np.array(hitrate)}
